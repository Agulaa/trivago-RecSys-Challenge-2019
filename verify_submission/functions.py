...
def check_duplicates(df):
    ...
    # print(df[df['session_id'].duplicated()])
    # print(len(df['session_id'].unique()))
    # print(len(df['session_id']))
...
def check_columns(df):
    ...
    #check_cols = not set(df.columns).difference(SUBMISSION_COLUMNS)
    # print(check_cols)
...
def check_sessions(df_subm, df_test):
    ...
    #check_sess =  not set_test_sessions.difference(set_subm_sessions)
